...
export async function sendFromERC20Bridged(receivingAccount: AccountAddress, amount: BigNumber): Promise<void> {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  if (process.argv.length < 3) {
    console.error(`Usage: sell <val> <account-index|address>`);
